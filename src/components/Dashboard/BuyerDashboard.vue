      ...
      <div class="bg-white/95 backdrop-blur-xl rounded-2xl shadow-xl border border-white/30 p-5 transition-all hover:shadow-2xl">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-xl font-bold text-gray-900 mt-1">{{ stats.totalOrders || 0 }}</p>
      ...
      <div class="bg-white/95 backdrop-blur-xl rounded-2xl shadow-xl border border-white/30 p-5 transition-all hover:shadow-2xl">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-xl font-bold text-gray-900 mt-1">{{ stats.pendingOrders || 0 }}</p>
            <div class="mt-2">
      ...
      <div class="bg-white/95 backdrop-blur-xl rounded-2xl shadow-xl border border-white/30 p-5 transition-all hover:shadow-2xl">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-xl font-bold text-gray-900 mt-1">₱{{ formatNumber(stats.totalSpent || 0) }}</p>
            <div class="mt-2 flex items-center text-xs text-gray-500">
              <span class="inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium bg-green-100 text-green-800">
                <svg class="-ml-0.5 mr-0.5 h-3 w-3 text-green-500" fill="currentColor" viewBox="0 0 20 20">
      ...
      <div class="bg-white/95 backdrop-blur-xl rounded-2xl shadow-xl border border-white/30 p-5 transition-all hover:shadow-2xl">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-xl font-bold text-gray-900 mt-1">{{ stats.totalMessages || 0 }}</p>
            <div class="mt-2">
        ...
        <div class="flex items-center justify-between">
          <div>
            ...
            <div class="flex items-center mt-1">
              <span class="text-xl font-bold text-gray-900">{{ stats.rating || '0.0' }}</span>
              <div class="flex ml-2">
          ...
          <div class="p-6">
              ...
              <div>
                ...
                <div class="h-64 bg-gray-50 rounded-lg flex items-center justify-center">
                  <div class="text-center">
                    ...
                    <p class="mt-2 text-sm text-gray-500">Chart will load here</p>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div class="h-64 bg-gray-50 rounded-lg flex items-center justify-center">
                  <div class="text-center">
                    ...
                    <p class="mt-2 text-sm text-gray-500">Breakdown chart will load here</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="mt-6 grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div class="bg-gray-50 rounded-lg p-4">
                ...
                <p class="text-lg font-bold text-gray-900 mt-1">₱{{ formatNumber(stats.averagePrice || 0) }}</p>
                <p class="text-xs text-gray-500 mt-1">+{{ stats.priceGrowth || 0 }}% from last month</p>
              </div>
              <div class="bg-gray-50 rounded-lg p-4">
                <p class="text-xs font-medium text-gray-500">Orders per Month</p>
                <p class="text-lg font-bold text-gray-900 mt-1">{{ stats.ordersPerMonth || 0 }}</p>
                <p class="text-xs text-gray-500 mt-1">+{{ stats.orderFrequencyGrowth || 0 }}% from last month</p>
              </div>
              <div class="bg-gray-50 rounded-lg p-4">
                <p class="text-xs font-medium text-gray-500">Preferred Category</p>
                <p class="text-lg font-bold text-gray-900 mt-1">{{ stats.preferredCategory || 'Cattle' }}</p>
                <p class="text-xs text-gray-500 mt-1">{{ stats.categoryPercentage || 42 }}% of purchases</p>
              </div>
            </div>
          </div>
        </div>
                ...
                <tbody class="bg-white divide-y divide-gray-200/50">
                  <tr v-if="tableData.length === 0" class="hover:bg-gray-50/80 transition-colors">
                    <td colspan="7" class="px-6 py-8 text-center text-gray-500">
                      ...
                      <p class="mt-2">No purchase history found</p>
                    </td>
                  </tr>
                  <tr v-for="(item, index) in tableData.slice(0, 3)" :key="index" class="hover:bg-gray-50/80 transition-colors">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                      <div class="flex items-center">
                        <div class="h-10 w-10 flex-shrink-0">
                          <img class="h-10 w-10 rounded-full object-cover" :src="item.image" :alt="item.livestock" />
                        </div>
                        <div class="ml-4">
                          <div class="text-sm font-medium text-gray-900">{{ item.livestock }}</div>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                      <span class="text-sm text-gray-900 font-semibold">{{ item.amount }}</span>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      {{ item.date }}
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                      <span :class="item.statusClass" class="px-2 py-1 text-xs font-medium rounded-full">{{ item.status }}</span>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-center">
                      <button class="text-emerald-600 hover:text-emerald-900 hover:bg-emerald-50 p-2 rounded-full transition-all duration-200" title="View Details">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <div>
                ...
                <div class="h-48 bg-gray-50 rounded-lg flex items-center justify-center">
                  <div class="text-center">
                    ...
                    <p class="mt-2 text-sm text-gray-500">Analytics chart will load here</p>
                  </div>
                </div>
              </div>
              <div>
                <h4 class="text-xs font-medium text-gray-700 mb-3">Status Overview</h4>
                <div class="space-y-3">
                  <div class="flex items-center">
                    ...
                    <div class="flex-1">
                      <div class="flex justify-between text-xs">
                        ...
                        <span class="text-gray-500">
                          {{ stats.statusBreakdown?.completed || 0 }} ({{ Math.round(((stats.statusBreakdown?.completed || 0) / (stats.totalOrders || 1)) * 100) }}%)
                        </span>
                      </div>
                      <div class="w-full bg-gray-200 rounded-full h-1.5 mt-1">
                        <div class="h-1.5 rounded-full bg-green-500" :style="`width: ${Math.round(((stats.statusBreakdown?.completed || 0) / (stats.totalOrders || 1)) * 100)}%`"></div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div class="flex items-center">
                    ...
                    <div class="flex-1">
                      <div class="flex justify-between text-xs">
                        ...
                        <span class="text-gray-500">
                          {{ stats.statusBreakdown?.processing || 0 }} ({{ Math.round(((stats.statusBreakdown?.processing || 0) / (stats.totalOrders || 1)) * 100) }}%)
                        </span>
                      </div>
                      <div class="w-full bg-gray-200 rounded-full h-1.5 mt-1">
                        <div class="h-1.5 rounded-full bg-yellow-500" :style="`width: ${Math.round(((stats.statusBreakdown?.processing || 0) / (stats.totalOrders || 1)) * 100)}%`"></div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div class="flex items-center">
                    ...
                    <div class="flex-1">
                      <div class="flex justify-between text-xs">
                        ...
                        <span class="text-gray-500">
                          {{ stats.statusBreakdown?.shipped || 0 }} ({{ Math.round(((stats.statusBreakdown?.shipped || 0) / (stats.totalOrders || 1)) * 100) }}%)
                        </span>
                      </div>
                      <div class="w-full bg-gray-200 rounded-full h-1.5 mt-1">
                        <div class="h-1.5 rounded-full bg-blue-500" :style="`width: ${Math.round(((stats.statusBreakdown?.shipped || 0) / (stats.totalOrders || 1)) * 100)}%`"></div>
                      </div>
                    </div>
                  </div>
                </div>
        ...
        <div class="bg-white/95 backdrop-blur-xl rounded-2xl shadow-xl overflow-hidden border border-white/30 transition-all hover:shadow-2xl">
          <div class="px-6 py-4 border-b border-gray-200/50 bg-gradient-to-r from-green-600 to-emerald-600">
            <div class="flex items-center justify-between">
              ...
              <router-link to="/messages" class="text-xs font-medium text-white hover:text-emerald-100 flex items-center">
                View All
                <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3 ml-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
                </svg>
          ...
          <div class="divide-y divide-gray-200/50">
            <div v-if="messages.length === 0" class="p-6 text-center text-gray-500">
              <svg class="mx-auto h-12 w-12 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z" />
              </svg>
            ...
            <div v-else>
              <ul class="divide-y divide-gray-200/50">
                <li v-for="message in messages.slice(0, 2)" :key="message.id" class="px-6 py-4 hover:bg-gray-50/50 transition-colors">
                  <div class="flex items-start">
                    <img class="h-8 w-8 rounded-full" :src="message.avatar" :alt="message.name" />
                    <div class="ml-3 flex-1">
                      <div class="flex items-center justify-between">
                        <h4 class="text-sm font-medium text-gray-900">{{ message.name }}</h4>
                        <span class="text-xs text-gray-500">{{ message.time }}</span>
                      </div>
                      <p class="text-xs text-gray-600 mt-1 truncate">{{ message.message }}</p>
                      <div class="mt-2 flex space-x-2">
                        <button class="text-xs font-medium text-emerald-600 hover:text-emerald-700">Reply</button>
                        <button class="text-xs font-medium text-gray-600 hover:text-gray-700">View Order</button>
                      </div>
                    </div>
