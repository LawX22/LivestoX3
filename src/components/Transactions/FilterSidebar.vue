<!-- components/Transactions/FilterSidebar.vue -->
<template>
  <div :class="`${isExpanded ? 'w-full md:w-52 lg:w-56' : 'w-14'} bg-white/95 backdrop-blur-xl border-r border-white/40 shadow-lg relative transition-all duration-300 ease-in-out`">
    ...
    <!-- Expanded View -->
    <div v-if="isExpanded" class="relative h-full flex flex-col">
      <!-- Header -->
      <div class="flex justify-between items-center p-3 border-b border-green-100/50">
        ...
        <div class="flex gap-1">
          <button 
            @click="$emit('reset')" 
            class="px-2 py-1 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-md text-xs font-medium transition flex items-center gap-1 cursor-pointer"
          >
      ...
      <div class="sticky top-0 z-10 bg-white/90 backdrop-blur-sm border-b border-green-100/50 p-3 shadow-sm">
        ...
        <div v-if="hasActiveFilters" class="bg-gradient-to-r from-green-50 to-emerald-50 rounded-md p-2 border border-green-200">
          <div class="flex flex-wrap gap-1">
            ...
            <span v-if="isBuyerView" v-for="farmer in localFilters.farmers" :key="farmer" class="px-1.5 py-0.5 bg-blue-100 text-blue-800 text-[10px] rounded-full font-medium">{{ farmer }}</span>
          </div>
        </div>
      </div>
      ...
      <div class="flex-1 overflow-y-auto p-3">
        <div class="space-y-3">
          ...
          <!-- Farmer Filter (Buyer View Only) -->
          <div v-if="isBuyerView && uniqueFarmers.length > 0" class="bg-white/80 backdrop-blur-sm rounded-lg p-2 border border-green-100">
            <h3 class="text-xs font-semibold text-gray-700 mb-2 flex items-center gap-1">
            ...
            <div class="space-y-1 max-h-32 overflow-y-auto">
              <label v-for="farmer in uniqueFarmers" :key="farmer" class="flex items-center gap-1.5 text-xs text-gray-700 hover:bg-blue-50/50 p-1 rounded cursor-pointer">
                <input 
                  type="checkbox" 
                  :value="farmer" 
                  v-model="localFilters.farmers" 
                  class="h-3 w-3 text-blue-600 rounded border-gray-300 focus:ring-blue-500 cursor-pointer"
                  @change="updateFilters"
                >
                <span class="truncate">{{ farmer }}</span>
              </label>
            </div>
          </div>
