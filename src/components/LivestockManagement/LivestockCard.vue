...
<template>
  ...
  <div v-if="displayMode === 'card'"
    ...
    <div class="p-4 flex-1 flex flex-col">
      ...
      <!-- Action Buttons - Different for auction vs normal -->
      <div v-if="animal.isAuction" class="flex gap-2">
        <!-- Edit Button for Auction -->
        <button @click.stop="$emit('editListing', animal)"
          class="flex-1 px-3 py-2 bg-gradient-to-r from-gray-600 to-gray-700 hover:from-gray-700 hover:to-gray-800 text-white text-sm font-bold rounded-md transition-all flex items-center justify-center gap-2 shadow hover:shadow-md border border-gray-500/20">
        ...
        <!-- Delete Button for Auction -->
        <button @click.stop="$emit('deleteListing', animal)"
      ...
      <div v-else class="flex gap-2">
        <!-- Edit Button for Regular Listing -->
        <button @click.stop="$emit('editListing', animal)"
          class="flex-1 px-3 py-2 bg-gradient-to-r from-blue-600 to-indigo-700 hover:from-blue-700 hover:to-indigo-800 text-white text-sm font-bold rounded-md transition-all flex items-center justify-center gap-2 shadow hover:shadow-md border border-blue-500/20">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
              d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
          </svg>
        ...
        <!-- Delete Button for Regular Listing -->
        <button @click.stop="$emit('deleteListing', animal)"
          class="flex-1 px-3 py-2 bg-gradient-to-r from-red-600 to-red-700 hover:from-red-700 hover:to-red-800 text-white text-sm font-bold rounded-md transition-all flex items-center justify-center gap-2 shadow hover:shadow-md border border-red-500/20">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
  ...
  <!-- Table Row View -->
  <tr v-else-if="displayMode === 'table'" class="hover:bg-gray-50/50 transition-colors duration-200">
    <!-- Image -->
    <td class="px-6 py-4 whitespace-nowrap">
      <div class="relative w-16 h-16 rounded-lg overflow-hidden cursor-pointer group" @click="$emit('openModal', animal)">
        ...
        <div class="absolute -top-1 -right-1">
          ...
          <span v-else :class="`px-1.5 py-0.5 text-xs font-bold rounded-full shadow-md ${getStatusClass(animal.status)}`">
            {{ animal.status === 'Available' ? '✓' : animal.status.charAt(0) }}
          </span>
        </div>
      </div>
    </td>
    ...
    <td class="px-6 py-4">
      <div class="flex flex-col">
        <div class="text-sm font-bold text-gray-900">{{ animal.breed }} {{ animal.type }}</div>
        <div class="flex items-center gap-2 mt-1">
          <span class="text-xs text-gray-600 bg-blue-50 px-2 py-0.5 rounded-md">{{ animal.gender }}</span>
          <span class="text-xs text-gray-600 bg-purple-50 px-2 py-0.5 rounded-md">{{ animal.age }}</span>
        </div>
      </div>
    </td>
    ...
    <td class="px-6 py-4">
      <div class="flex items-center">
        <div class="w-8 h-8 rounded-full border border-gray-200 shadow-sm overflow-hidden mr-3">
          <img :src="animal.farmer.avatar" :alt="animal.farmer.name" class="w-full h-full object-cover" />
        </div>
        <div>
          <div class="text-sm font-medium text-gray-900">{{ animal.farmer.farmName || animal.farmer.name }}</div>
          <div class="text-xs text-gray-500">{{ animal.farmer.name }}</div>
        </div>
      </div>
    </td>
    ...
    <td class="px-6 py-4 whitespace-nowrap">
      <div class="flex items-center gap-2">
        <!-- Edit Button -->
        <button @click="$emit('editListing', animal)"
          class="px-2 py-1 bg-blue-600 hover:bg-blue-700 text-white text-xs font-medium rounded transition-colors flex items-center gap-1 shadow hover:shadow-md">
        ...
        <!-- Delete Button -->
        <button @click="$emit('deleteListing', animal)"
          class="px-2 py-1 bg-red-600 hover:bg-red-700 text-white text-xs font-medium rounded transition-colors flex items-center gap-1 shadow hover:shadow-md">
          <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
const props = defineProps<{
  ...
  weightUnit?: string;
  displayMode?: 'card' | 'table'; // NEW PROP
}>();
