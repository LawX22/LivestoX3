            ...
            <table class="min-w-full divide-y divide-gray-200">
              ...
              <tbody class="bg-white divide-y divide-gray-200">
                ...
                >
                  <td class="px-6 py-4 whitespace-nowrap">
                    <div class="flex items-center">
                      <div class="flex-shrink-0 h-10 w-10 relative">
                        ...
                        >
                        <span v-if="isNewRequest(request)" class="absolute top-0 right-0 block h-2.5 w-2.5 rounded-full ring-2 ring-white bg-red-500"></span>
                      </div>
                      <div class="ml-4">
                  ...
                  <td class="px-6 py-4">
                    <div class="text-sm font-medium text-gray-900">{{ request.farmDetails?.farmName || 'N/A' }}</div>
                ...
                <tr v-if="paginatedRequests.length === 0">
                  <td colspan="5" class="px-6 py-4 text-center text-gray-500">
                    <div class="flex flex-col items-center justify-center py-8">
                      ...
                      <h3 class="mt-2 text-sm font-medium text-gray-700">No upgrade requests found</h3>
                ...
                <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                  ...
                  <template v-for="page in visiblePages" :key="page">
                    <button
                      ...
                      :class="[page === currentPage ? 'z-10 bg-emerald-50 border-emerald-500 text-emerald-600' : 'bg-white border-gray-300 text-gray-500 hover:bg-gray-50', 'relative inline-flex items-center px-4 py-2 border text-sm font-medium']"
                    >
                      {{ page }}
                    </button>
                  </template>
  ...
  setup() {
    ...
    const approvedTodayCount = computed(() => {
      ...
      return filteredRequests.value.filter(r => 
        r.status === 'approved' && r.date && r.date.startsWith(today)
      ).length
    })
